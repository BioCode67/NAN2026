...
export const STAGE = {
  ...
  BLAST_MARGIN: 260,
  /**
   * 이 Y좌표 위로 **빠르게 상승 중**이면 장외 → 별이 된다 (스타 KO).
   *
   * 높이만으로 자르면 달 중력에서 2단 점프(정점 약 -240)로 제 발로
   * 죽을 수 있어, 아래 STAR_KO_MIN_VY 와 함께 판정한다. 점프 정점은
   * 속도가 0에 가까우니 절대 못 넘고, 큰 넉백에 실려 갈 때만 넘는다.
   */
  BLAST_TOP: -220,
  /** 상단 장외로 인정하는 최소 상승 속도 (px/s) */
  STAR_KO_MIN_VY: 450,
  /**
   * 상단 장외는 이 시간 안에 맞은 적이 있어야 인정한다 (ms).
   *
   * 넉백에 실려 올라가는 중인지, 제 발로 뛴 것인지를 가르는 조건이다.
   */
  STAR_KO_LAUNCH_MS: 1200,
  /**
   * 낙사를 "마지막으로 때린 사람"의 격추로 인정하는 시간 (ms).
   *
   * 이 시간이 지나서 떨어지면 밀어낸 사람과 무관한 실족이라 보고
   * 자멸로 기록한다.
   */
  RINGOUT_CREDIT_MS: 4000,
