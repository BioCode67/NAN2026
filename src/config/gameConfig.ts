...
export const FIGHTER = {
   ...
   * 공중 조작 감쇠.
   *
   * 0.82 는 뜬 순간 몸이 무거워진다는 뜻이었다. 공중이 이 게임에서 싸움이
   * 일어나는 곳인데 거기서 가장 굼떴다 — "답답하다"의 절반이 여기 있었다.
   */
  AIR_CONTROL: 0.94,
  /**
   ...
   * 대시 속도 배율.
   *
   * 3.4 × 280ms 로는 한 번 대시해 봐야 230px 남짓 — 1920px 무대에서
   * 여덟 번을 밟아야 반대편에 닿는다. 대시는 "붙거나 빠지거나"를 정하는
   * 수인데, 그 거리로는 어느 쪽도 정해지지 않고 그냥 조금 미끄러졌다.
   * 4.6 × 310ms = 350px 면 한 번에 판단이 선다.
   */
  DASH_SPEED_MUL: 4.6,
  /** 대시 재사용 대기 (ms) — 다음 판단까지 기다리는 시간을 줄였다 */
  DASH_COOLDOWN: 400,
  /** 방어 시 피해 배율 */
 ...
 * 기질마다의 숫자와 사람이 읽을 설명.
 *
 * 설명은 선택 화면에 그대로 나간다 — 골라 보기 전에 "이 사람은 이렇게
 * 움직이는구나"가 읽혀야 한 명을 파고들 이유가 생긴다.
 */
export const MOVE_TRAITS = {
  plain: {
    name: '표준',
    icon: '🚶',
    desc: '특별한 공중 재주가 없다 — 대신 땅에 발을 붙이고 있는 시간이 짧다',
    /**
     * 전투 화면 안내에 뜨는 한 줄 — **어떤 버튼으로 되는가**까지 적는다.
     *
     * 전용기는 조건이 붙어 있어서(떨어지는 중에, 벽에 밀렸을 때, 공중에서)
     * 우연히 눌러서는 평생 안 나온다. 선택 화면 설명은 고를 때 한 번 보고
     * 잊히므로, 판이 시작할 때 자기 것 한 줄을 다시 보여 줘야 실제로 손이
     * 간다.
     */
    only: 'K 를 꾹 눌러 모으는 속도가 남보다 빠르다 — 상대가 "아직 안 찼겠지" 하는 박자를 잡는다',
    /**
     * 모으는 속도 배율 — 이 기질만 되는 것.
     *
     * 나머지 넷은 공중에서 자기만의 수를 하나씩 갖는다. 표준에게만 아무것도
     * 없으면 "기질이 없는 캐릭터"가 되고, 그건 고르는 재미가 아니라 손해다.
     * 그래서 반대쪽에 준다 — 공중에 답이 없는 대신 **땅에서 버티고 모으는
     * 시간이 짧다**. 같은 900ms 짜리 풀차지를 640ms 에 끝내므로, 상대가
     * "아직 안 찼겠지" 하고 들어오는 그 한 박자를 이 캐릭터만 잡는다.
     */
    chargeMul: 1.4,
  },
  glide: {
    name: '활공',
    icon: '🪂',
    desc: '떨어질 때 점프를 누르고 있으면 천천히 내려온다 — 공중에 오래 머문다',
    /** 하강 속도를 이 값으로 눌러 둔다 (px/s) */
    fallCap: 180,
    /**
     * 활공 중에 공격하면 활공을 끊고 내리꽂는다 (급습).
     *
     * 이 캐릭터만 되는 것이 필요했다. 활공만으로는 "느리게 떨어진다"에서
     * 끝나고, 그건 도망 수단이지 무기가 아니다. 떠 있다가 **아무 공격이나**
     * 누르면 그 자리에서 돌처럼 떨어지면, 아래에서 기다리는 쪽은 언제
     * 떨어질지 읽을 수 없다 — 떠 있는 시간 자체가 압박이 된다.
     *
     * 점프를 떼면 평범한 공중 공격이 나가므로, 급습은 **고르는 것**이다.
     */
    diveSpeed: 1250,
    diveMul: 1.3,
    only: '떨어지는 중 SPACE 를 누른 채 J/K → 활공을 끊고 그대로 내리꽂는다 (급습)',
  },
  plunge: {
    name: '급강하',
    icon: '🪨',
    desc: '남보다 무겁게, 떨어질수록 빨라지며 내려온다',
    /** 하강 가속 배율 */
    fallMul: 1.5,
    /** 착지 충격이 터지는 최소 낙하 속도 */
    slamAt: 900,
    slamRange: 130,
    slamDamage: 7,
    only: '높은 데서 떨어져 착지하면 발밑이 터진다 — 쫓아가는 게 아니라 위에서 덮는다',
  },
  wallkick: {
    name: '벽 차기',
    icon: '🧱',
    desc: '무대 밖으로 밀려도 벽을 한 번 차고 되돌아온다 — 한 체공에 한 번',
    /** 벽을 찰 때 안쪽으로 밀어내는 속도 */
    kickX: 520,
    kickY: -620,
    /** 벽을 찬 뒤 이 시간 안의 첫 공격이 세진다 (ms) */
    boostMs: 2000,
    /** 그 한 방의 피해·넉백 배율 */
    boostMul: 1.45,
    only: '벽을 찬 뒤 2초 안에 내는 첫 한 방이 1.45배로 세진다 — 몰렸다가 뒤집는 자리',
  },
  drift: {
    name: '표류',
    icon: '🌀',
    desc: '공중에서 관성이 오래 남아 미끄러지듯 흐른다 — 궤적이 길고 부드럽다',
    /** 공중 조작이 속도를 바꾸는 정도 (1 = 지상과 같음) */
    control: 0.45,
    /** 공중 관성 유지 — 매 프레임 남는 비율 */
    keep: 0.995,
    /** 한 체공에 쓸 수 있는 공중 대시 횟수 (기본은 1) */
    airDashes: 2,
    only: '공중 대시를 한 체공에 두 번 — 남들은 한 번이다. 관성이 남아 더 멀리 간다',
  },
} as const;
