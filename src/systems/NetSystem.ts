...
export interface NetSnapshot {
  ...
  /**
   * 파이터별
   * [x, y, vx, vy, facing, 살아있음, 주가, 포즈번호,
   *  스킬쿨다운(0~100), 고유자원 스택, 고유자원 신호(0/1), 든 아이템 번호(-1 없음)]
   */
  f: number[][];
  /**
   * 이번 구간에 일어난 타격
   * [x, y, 색, 마무리인가, 피해자 자리, 반응 번호, 리듬판정, 공격자 자리, 연타 수]
   */
  hit?: number[][];
  /** 프롬프트 오브 [x, y, 남은체력 0~100, 상태 0생존/1파괴/2도망]. 없으면 없음 */
  orb?: number[];
  /** 떨어져 있는 아이템들 [x, y, 종류번호] */
  item?: number[][];
  /** 날아가는 투사체들 [x, y, 색, 크기] */
  pj?: number[][];
  /** 이번 구간의 상장폐지 [피해자 자리, 격추자 자리(-1 자멸)] */
  ko?: number[][];
  /** 상장폐지된 사람들의 유령 [자리번호, x, 던질 수 있는가] */
  gh?: number[][];
  /** 서든데스가 발동했으면 1 — 참가자도 같은 연출을 켠다 */
  sd?: number;
  /** 판이 끝났으면 이긴 자리 번호 (-1 = 전원 탈락) */
  over?: number;
  /** 판이 끝났을 때의 전적 [준피해, 맞은피해, KO, 적중, 최고한방] + 그 기술 이름 */
  stat?: Array<[number, number, number, number, number, string]>;
}
