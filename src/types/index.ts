...
export interface CharacterConfig {
  ...
  /**
   * 이동 기질 — **이 캐릭터가 어떻게 움직이는가.**
   *
   * ── 왜 따로 두는가 ──────────────────────────────────────────────
   * 스무 명이 이름과 숫자만 다르고 **똑같이 뛰고 똑같이 떨어졌다.**
   * 속도·점프 높이는 숫자라서 손에 잘 안 잡히는데, 한 명을 골라 익숙해지는
   * 재미는 그 인물만의 움직이는 법을 몸에 붙이는 데서 온다.
   *
   * 시그니처(공격 자원)와 직각으로 둔다 — 다섯 가지 자원 × 다섯 가지
   * 기질로 스무 명이 저마다 다른 자리에 놓인다.
   */
  move: MoveTrait;
 ...
 * 이동 기질 다섯 갈래.
 *
 * 전부 **공중에서** 갈린다. 지상 이동은 이미 speed 로 갈려 있고, 난투에서
 * 판이 뒤집히는 순간은 거의 공중에 있다 — 띄우고 쫓아가고 떨어뜨리는 그
 * 몇 초가 이 게임의 가장 재미있는 구간이다.
 */
export type MoveTrait =
  /** 표준 — 특별한 기질이 없다 (기준점이 있어야 나머지가 특별해진다) */
  | 'plain'
  /**
   * 활공 — 떨어질 때 점프를 누르고 있으면 천천히 내려온다.
   * 공중에 오래 머물며 위에서 판을 내려다보는 캐릭터가 된다.
   */
  | 'glide'
  /**
   * 급강하 — 무겁게 떨어지고, 착지 순간 발밑에 충격이 퍼진다.
   * 위에서 아래로 내리누르는 압박이 이 캐릭터의 언어다.
   */
  | 'plunge'
  /**
   * 벽 차기 — 무대 양 끝 벽을 한 번 차고 되돌아온다.
   * 장외로 밀려도 살아 돌아오는 끈질김.
   */
  | 'wallkick'
  /**
   * 표류 — 공중에서 관성이 오래 남아 미끄러지듯 흐른다.
   * 치고 빠지는 궤적이 길고 부드럽다.
   */
  | 'drift';
...
export interface GimmickSpec {
  ...
  keywords: string[];
  /**
   * 그림 생성 서버에 보낼 **장면 설명**(영어).
   *
   * 플레이어가 쓴 한국어 한 줄만 보내면 그림이 잘 안 나온다 — 한 줄에는
   * 장면이 부족하고, 모델은 대부분 영어로 배웠다. 해석기가 이 문장을
   * 무엇으로 읽었는지가 곧 "무슨 일이 벌어졌는가"이므로, 그 장면을 여기
   * 적어 두고 앞세워 보낸다. 플레이어의 말은 뒤에 그대로 붙인다.
   */
  art: string;
}
