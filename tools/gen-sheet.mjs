...
- 24장 전부 같은 캐릭터로 보여야 한다. 옷 색·머리 모양·무기 형태를 프레임마다 바꾸지 말 것
...
[프레임 24개, 이 순서로]
1행: IDLE(대기), WALK(걷기), RUN_A(달리기1), RUN_B(달리기2),
     JUMP(솟아오르는 중, 무릎 접음), FALL(떨어지는 중, 다리 뻗음)
2행: ATTACK_J(짧은 견제 공격),
     ATTACK_J_UP(${m.jUp}),
     ATTACK_J_DOWN(${m.jDown}),
     ATTACK_K(크게 휘두르는 강공격, 궤적 잔상),
     ATTACK_K_UP(${m.kUp}),
     ATTACK_K_DOWN(${m.kDown})
3행: AIR_J(공중에서 앞으로 내지르는 짧은 공격),
     AIR_K(공중에서 몸을 회전시키며 크게 휘두름),
     AIR_DIVE(${m.dive}),
     SKILL_L(${m.skill} — 기를 모으는 순간),
     SKILL_L2(같은 기술을 방출하는 순간, 캐릭터 포함),
     SKILL_FX(방출된 에너지 이펙트만. 캐릭터를 그리지 말 것)
4행: HIT(피격), KNOCKBACK(날아감), GUARD(무기를 세워 막기), DASH(질주),
...
if (!dry && !save && ids.length) {
  console.log(
    `\n다음 단계: 전처리 로그에 "총 24개"가 찍혔는지 확인하고\n` +
      ...
      `프레임 수가 24가 아니면 시트를 다시 뽑는 편이 빠릅니다.\n` +
